...
def calculateAccuracies(solvedDict, testOutFileName, outFile):
    ...
    with open(testOutFileName, encoding='utf-8' ) as data:
        lineCount = 0
        for line in data.read().split("\n"):
            if len(line) > 2:
                prevStudentArray = []
...
def main():
    ...
    testFile = sys.argv[2]
    dialogDict, numDialogDict, uniqueDialogDict, countUniqueDialogDict = parse_file(trainFile)
    probSensesDict = probabilitiesOfSenses(numDialogDict)
    wordsAndTestIDsDict = extractTestData(testFile)
    ...
    calculateAccuracies(solvedDict, testFile, outFile)
